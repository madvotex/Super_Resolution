...
# ==============================================================================
# BLOCK 3: DISEASE-AWARE DATASET LOADER (CLAHE REMOVED FROM TARGET)
# ==============================================================================
...
# ==============================================================================
# BLOCK 5: LOSS FUNCTIONS (SSIM INTEGRATED)
# ==============================================================================
...
def vessel_aware_structure_loss(sr, hr, l1_fn, ssim_fn):
    l1_loss = l1_fn(sr, hr)
    ssim_loss = ssim_fn(sr, hr)
    
    sr_g, hr_g = sr[:, 1, :, :], hr[:, 1, :, :]
    green_loss = torch.abs(sr_g - hr_g).mean()
    
    return 0.84 * ssim_loss + 0.16 * l1_loss + 1.5 * green_loss

# ==============================================================================
# BLOCK 6: TRAINING LOOP (PURE WARMUP + HYBRID GAN)
# ==============================================================================
def train():
    BATCH_SIZE = 4
    PATCH_SIZE = 48 
    ...
    # --- STAGE 1: EXTENDED WARMUP (Pure L1 Foundation) ---
    WARMUP_EPOCHS = 15 
    print(f"\n{'='*10} STAGE 1: EXTENDED WARMUP (Structure) {'='*10}")
    ...
    for epoch in range(WARMUP_EPOCHS):
        ...
        for lr, hr in loop:
            ...
            with torch.cuda.amp.autocast():
                sr = G(lr)
                # CRITICAL FIX: Pure L1 for Warmup to prevent color/residual corruption
                loss = l1(sr, hr) 
    ...
    # --- STAGE 2: GAN (Texture Fine-Tuning) ---
    GAN_EPOCHS = 10 
    ...
    for epoch in range(GAN_EPOCHS):
        ...
        for lr, hr in loop:
            ...
            with torch.cuda.amp.autocast():
                ...
                content = vessel_aware_structure_loss(sr, hr, l1, ssim_loss_fn)
                perceptual = l1(vgg(sr_norm), vgg(hr_norm))
                adversarial = bce(fake_pred, torch.ones_like(fake_pred))
                edge = edge_loss_fn(sr, hr)
                
                # High content weight, low adversarial/perceptual for safety
                g_loss = 1.0 * content + 0.1 * perceptual + 0.001 * adversarial + 0.5 * edge
...
# ==============================================================================
# BLOCK 7: INFERENCE, INTERPOLATION & METRICS (MASKED EVALUATION)
# ==============================================================================
def net_interpolation(warmup_path, gan_path, alpha=0.8): 
...
def process_full_image(model, prefix="final"): 
    print(f"\n>>> Processing Full Image & Calculating Metrics ({prefix.upper()} model)...")
    ...
    # CRITICAL FIX: DYNAMIC FUNDUS MASKING
    mask = (hr_img.sum(axis=2) > 0.05).astype(np.float32)[..., np.newaxis]
    
    sr_img = sr_img * mask
    hr_img = hr_img * mask
    bic_img = bic_img * mask
    ...
    print(f"\n{'-'*40}")
    print(f"MASKED PERFORMANCE METRICS ({prefix.upper()})")
    print(f"{'-'*40}")
    print(f"Metric   | Bicubic (Base) | Medical SRGAN")
    print(f"---------|----------------|---------------------")
    print(f"PSNR     | {psnr_bic:.2f} dB       | {psnr_val:.2f} dB")
    print(f"SSIM     | {ssim_bic:.4f}         | {ssim_val:.4f}")
    print(f"LPIPS    | N/A            | {lpips_val:.4f}")
    print(f"{'-'*40}\n")
    
    fig, axes = plt.subplots(1, 3, figsize=(20, 7))
    axes[0].imshow(bic_img)
    axes[0].set_title(f"Bicubic (Masked)\nPSNR: {psnr_bic:.2f} | SSIM: {ssim_bic:.3f}", fontsize=12)
    axes[0].axis('off')
    
    axes[1].imshow(sr_img)
    axes[1].set_title(f"Medical SRGAN (Masked)\nPSNR: {psnr_val:.2f} | SSIM: {ssim_val:.3f} | LPIPS: {lpips_val:.3f}", 
                      fontsize=12, fontweight='bold', color='darkblue')
    axes[1].axis('off')
    
    axes[2].imshow(hr_img)
    axes[2].set_title("Ground Truth (Masked)", fontsize=12)
    axes[2].axis('off')
    
    plt.tight_layout()
    out_path = os.path.join(OUTPUT_DIR, f"full_image_metrics_{prefix}.png") 
...
if __name__ == "__main__":
    ...
    print("\n>>> Starting Fully Optimized Retraining Pipeline...")
    train() 
    
    warmup_path = os.path.join(MODEL_DIR, "G_warmup.pth")
    gan_path = os.path.join(MODEL_DIR, "G_final.pth")
    
    print("\n>>> Evaluating WARMUP Baseline (Pure Structure)...")
    warmup_model = ESRGAN_Generator().to(device)
    warmup_model.load_state_dict(torch.load(warmup_path))
    warmup_model.eval()
    process_full_image(warmup_model, prefix="warmup")
    
    print(f"\n>>> Interpolating Models (alpha=0.8 for STRUCTURE priority)...")
    best_model = net_interpolation(warmup_path, gan_path, alpha=0.8) 
    best_model.eval()
    
    process_full_image(best_model, prefix="interpolated")