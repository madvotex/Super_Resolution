    ...
    for epoch in range(GAN_EPOCHS):
        ...
        for lr, hr in loop:
            ...
            with torch.cuda.amp.autocast():
                ...
                # --- CHANGED: STRUCTURE-FOCUSED LOSS WEIGHTS ---
                ...
                # High content weight, low adversarial/perceptual for structure retention
                g_loss = 1.0 * content + 0.1 * perceptual + 0.005 * adversarial + 0.5 * edge
...
# ==============================================================================
def net_interpolation(warmup_path, gan_path, alpha=0.8): # CHANGED: Alpha defaults to 0.8
...
def process_full_image(model, prefix="final"): # CHANGED: Added prefix parameter
    ...
    print(f"\n{'-'*30}")
    print(f"PERFORMANCE METRICS ({prefix.upper()})")
    print(f"{'-'*30}")
    ...
    plt.tight_layout()
    out_path = os.path.join(OUTPUT_DIR, f"full_image_metrics_{prefix}_2.png") # CHANGED: Variable save name
...
if __name__ == "__main__":
    ...
    print("\n>>> Starting Retraining Pipeline (Updated Loss Weights)...")
    # CHANGED: Force train to apply new loss functions
    train() 
    
    warmup_path = os.path.join(MODEL_DIR, "G_warmup.pth")
    gan_path = os.path.join(MODEL_DIR, "G_final.pth")
    
    # CHANGED: Evaluate the Warmup (L1/Structure) Baseline first
    ...
    # Evaluate Interpolated Final Model
    print(f"\n>>> Interpolating Models (alpha=0.8 for STRUCTURE priority)...")
    best_model = net_interpolation(warmup_path, gan_path, alpha=0.8) # CHANGED: Alpha is now 0.8
    best_model.eval()
